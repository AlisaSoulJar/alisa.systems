# Cucco Swarm: Industrial Polish & Mechanics (Phase 5)

## 1. Aesthetic Evolution (13 Rue del Percebe)
The visual identity of the Cucco Swarm simulation was refactored to achieve a "Comic / Flat Industrial" aesthetic:
...
- **Material Polish**: All `.glb` imports (Janitor, Rats, Cuccos) and procedural obstacles (Crates) are explicitly forced into absolute matte (`roughness = 1.0`, `metalness = 0.0`) by traversing the meshes on load. This removes the "plastic" sheen and grounds them into the industrial tile floor.
...
- **Spawn Logic**: Enemies have a 25% chance to drop a `Slime Puddle` upon death (`CuccoGameSystem._killEnemy`). Puddles persist for 12 seconds.
...
Added a layer of mechanical complexity for Elite units (e.g. `boss` or `cucco` classes).
...
- **GPU Cleanup**: Iterates over `CuccoEnvironmentFactory`'s `Map` caches, calling `.dispose()` on all procedural materials and using `scene.remove()` to clean the WebGL state.
