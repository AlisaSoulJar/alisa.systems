 ...
 *
 * El fallo que había: la mesa corría la escena media rejilla para centrar el
 ...
 *
 * No lo cazó nadie porque hasta hoy esta mesa tenía nueve mundos en las pruebas y
 * cero páginas en pantalla.
 */
{
    const escena = new THREE.Scene();
    const mesa = new MesaMundo(escena, {}, 1);
    mesa.pintar({ piezas: [{ x: 0, y: 0, alto: 0, t: 'nave' }], zonas: [], leyenda: {} });
    const m = [...mesa._piezas.values()][0];
 ...
 *
 * Es lo que hace que la mesa valga para las páginas con arte: ¡Busca! tiene
 * planetas con textura hechos por una factoría, y sin este enganche habría que
 * elegir entre el sustrato y lo que se ve. La mesa sigue llevando la posición, la
 ...
 */
{
    ...
    const mesa = new MesaMundo(escena, { nave: { malla: propia } }, 1);
    const sus = { piezas: [{ x: 3, y: 4, alto: 5, t: 'nave' }], zonas: [], leyenda: {} };
    mesa.pintar(sus);
    mesa.pintar(sus);   // segundo fotograma: NO se vuelve a fabricar
    const m = [...mesa._piezas.values()][0];
    if (veces !== 1) mal(`la figura del mundo se fabricó ${veces} veces, y tocaba una`);
        ...
        console.log('  ✓ la página pone la figura y la mesa la coloca desde el sustrato');
    }
}
 ...
 * escaneado pasaba por pieza nueva: la mesa fabricaba otra figura y escondía la
 ...
 */
{
    ...
    const escena = new THREE.Scene();
    const mesa = new MesaMundo(escena, ESTILO, 1);
    mesa.pintar(core.sustrato());
    const figuraDeP0 = [...mesa._piezas.values()].find(m => m.name === 'p0');
    ...
    if (!escaneados) {
        ...
        if (repetidas.length) {
            mal(`la mesa volvió a pedir figura para ${repetidas.map(([c]) => c).join(', ')}`
        ...
        const ahora = [...mesa._piezas.values()].find(m => m.name === 'p0');
        if (ahora !== figuraDeP0) mal('el planeta 0 cambió de figura sin morirse');

        /**
         * Y lo que se ve tiene que estar donde dice el sustrato. Si la mesa
        ...
        for (const p of sus.piezas) {
            const m = [...mesa._piezas.values()].find(x => x.name === p.cajon);
