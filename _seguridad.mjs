...
console.log(`1. ana se sienta: ${ana.estado} · recibe secreto: ${!!ana.d.secret}`);

const bea = await post('sentarse', { quien: 'bea', tipo: 'agente' });
console.log(`2. bea se sienta: ${bea.estado} · recibe secreto: ${!!bea.d.secret}`);
...
const suplantando = await post('jugar', { quien: 'ana', jugada: 'e2e4', secreto: bea.d.secret });
console.log(`5. jugar con el secreto de OTRA: ${suplantando.estado} — ${String(suplantando.d.error ?? '').slice(0, 60)}`);

const bien = await post('jugar', { quien: 'ana', jugada: 'e2e4', secreto: ana.d.secret });
console.log(`6. jugar con el secreto propio: ${bien.estado} · jugadas ${bien.d.moves}`);

const mirar = await (await fetch(`${BASE}/mesa/${SALA}?quien=bea`)).json();
const fuga = JSON.stringify(mirar).includes(ana.d.secret) || JSON.stringify(mirar).includes(bea.d.secreto ?? 'xxx');
