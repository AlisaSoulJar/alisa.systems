...
export class ProceduralRigging {
    ...
    static buildArchetypeSkeleton(type, renderGroup, detectedSlices = []) {
            ...
            if (topologicalBones && topologicalBones.length > 0) {
                ...
                // más atrás hasta la base de la cola, mientras la barriga (LB) acaba en las caderas.
                const UB     = [0, 3.0, -2.0]; // Upper Back (Coxis superior, nace directamente en el anillo de la cola)
                const LB     = [0, 1.5, -1.4]; // Lower Back (Pelvis inferior, nacen las patas)
                ...
                addBone(UB, LB); // Pelvis trasera
                addBone(MB, LM); // Costilla media
                addBone(UF, LF); // Costilla frontal (hombros)
                
                // === TAIL: Sale de UB (Arriba en el anillo trasero) ===
                addBone(UB, [0, 4.5, -2.2]);              // UB → Mid tail (subiendo)
                addBone([0, 4.5, -2.2], [0, 6.0, -2.5]);  // Mid tail → Tip

                // === NECK & HEAD: Base Cervical + Cuello + Cabeza ===
                const CERVICAL = [0, 3.3, 1.5];
                addBone(UF, CERVICAL);                         // Hombro espinal → Base cervical
                addBone(CERVICAL, [0, 4.0, 1.8]);             // Base cervical → Cráneo
                addBone([0, 4.0, 1.8], [0, 3.6, 2.3]);        // Cráneo → Hocico
                addBone([0, 4.0, 1.8], [-0.2, 4.4, 1.6]);     // Oreja L
                addBone([0, 4.0, 1.8], [0.2, 4.4, 1.6]);      // Oreja R

                // === HINDLEGS: Nacen de LB (Abajo) ===
                // L Branch
                addBone(LB, [-0.4, 1.5, -1.4]);                 // L Hip Width
                addBone([-0.4, 1.5, -1.4], [-0.5, 0.9, -0.6]);  // L Femur
                addBone([-0.5, 0.9, -0.6], [-0.5, 0.4, -1.5]);  // L Tibia
                addBone([-0.5, 0.4, -1.5], [-0.5, 0.1, -1.2]);  // L Metatarso
                addBone([-0.5, 0.1, -1.2], [-0.5, 0.0, -0.9]);  // L Paw
                
                // R Branch
                addBone(LB, [0.4, 1.5, -1.4]); 
                addBone([0.4, 1.5, -1.4], [0.5, 0.9, -0.6]);
                addBone([0.5, 0.9, -0.6], [0.5, 0.4, -1.5]);
                addBone([0.5, 0.4, -1.5], [0.5, 0.1, -1.2]);
                addBone([0.5, 0.1, -1.2], [0.5, 0.0, -0.9]);

                // === FORELEGS: Nacen de LF (hombro inferior) ===
                // L Branch
                addBone(LF, [-0.4, 1.5, 1.2]);                 // L Clavicle Width
                addBone([-0.4, 1.5, 1.2], [-0.5, 0.9, 1.0]);   // L Humerus
                addBone([-0.5, 0.9, 1.0], [-0.5, 0.4, 1.2]);   // L Forearm
                addBone([-0.5, 0.4, 1.2], [-0.5, 0.1, 1.2]);   // L Carpus
                addBone([-0.5, 0.1, 1.2], [-0.5, 0.0, 1.5]);   // L Paw
                
                // R Branch
                addBone(LF, [0.4, 1.5, 1.2]); 
                addBone([0.4, 1.5, 1.2], [0.5, 0.9, 1.0]);
                addBone([0.5, 0.9, 1.0], [0.5, 0.4, 1.2]);
                addBone([0.5, 0.4, 1.2], [0.5, 0.1, 1.2]);
                addBone([0.5, 0.1, 1.2], [0.5, 0.0, 1.5]);
     ...
     */
    static bindMesh(skeleton, rawBones, rootBones, renderGroup) {
        ...
        baseW.traverse(c => {
            if (c.isMesh) {
                ...
                // Root bones MUST be children of the SkinnedMesh before bind().
                // Ref: https://threejs.org/docs/#api/en/objects/SkinnedMesh
                if (!bonesAttached && rootBones && rootBones.length > 0) {
                    rootBones.forEach(rb => skinnedMesh.add(rb));
                    bonesAttached = true;
                    console.log('[bindMesh] ✅ Attached', rootBones.length, 'root bones to SkinnedMesh (canonical pattern)');
                }
